			...
			if (Object.keys(this._config).indexOf("schema") > -1) {
				Object.keys(this._config.schema).forEach((each_index) => {
					if (
						...
						if (typeof this._config.schema[each_index].default === "function") {
							self[each_index] = this._config.schema[each_index].default(this);
						} else {
				...
				if (
					...
					if (
						...
						params.Key = [
							...
							self[each_index],
							self[
								this._config.schema[each_index].key || this._config.hashKey
							] + ".json",
						]
							.filter(Boolean)
							.join("/");
					...
					putPromises.push(this._config.S3.putObject(params).promise());
				}
			});
		...
		constructor(collectionName, schema, opts) {
			...
			dbHandler.fetch = async (value) => {
				...
				var params = {
					...
					Key: [dbPrefix, this.config.collection, parameter, value + ".json"]
						.filter(Boolean)
						.join("/"),
				};
				...
				return new Promise((resolve, reject) => {
					...
					if (query) {
						...
						if (typeof query[key] === "boolean") {
							paramsPrefixes = [dbPrefix, this.config.collection, `${key}/`];
						} else {
							paramsPrefixes = [
								dbPrefix,
								this.config.collection,
					...
					try {
						...
						for (var key of this.config.indexes) {
							...
							if (object[key]) {
								...
								if (
									this.config.schema[key] &&
									this.config.schema[key].canRepeat
								) {
									deleteParams[key] = `${object[key]}/${
										object[this.config.hashKey]
									}`;
								} else {
									deleteParams[key] = object[key];
								}
			...
			dbHandler.ls = async (value, opts = {}) => {
				...
				if (typeof value === "object") {
					...
					if (this.config.schema[key]) {
						if (
							this.config.schema[key].composed &&
						...
						return {
							error: `"${key}" is not defined in ${collection}'s schema`,
						};
					}
				}
